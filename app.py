...
# for_sale_inventory_df2 = for_sale_inventory_df["StateName"].fillna("UnitedStates")
# for_sale_inventory_df2 = for_sale_inventory_df["StateName"].drop_duplicates()
# for_sale_inventory_df2 = for_sale_inventory_df2.sort().tolist()
...
with ui.sidebar():
    ...
    ui.input_slider("date_range","Filter be Date Range",
                    min = string_to_date("2018-3-31"),
                    max = string_to_date("2024-6-30"),
                    value = [string_to_date(x) for x in ["2018-3-31","2018-3-31"]])
...
with ui.navset_card_underline(title= "Median List Price"):
    ...
    with ui.nav_panel("Plot", icon = icon_svg("chart-line")):
        ...
        def list_price_plot():
            ...
            # we are doing this so that when someone selects a statename
            # they are gonna filter this for that state name
...
with ui.navset_card_underline(title= "Home Inventory"):
    ...
    with ui.nav_panel("Plot", icon = icon_svg("chart-line")):
        ...
        def for_sale_plot():
            ...
            # Creating Visualization using Ployly
            df2_melted = filter_by_date(df2_melted, input.date_range())
...
with ui.navset_card_underline(title= "New Listings"):
    ...
    with ui.nav_panel("Plot", icon = icon_svg("chart-line")):
        ...
        def listings_plot():
            ...
            # Creating Visualization using Ployly
            df3_melted = filter_by_date(df3_melted, input.date_range())
            if input.state() == "United States":
                df = df3_melted
            else:
